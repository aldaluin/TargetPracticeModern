...
[RequireComponent (typeof(ManedWolfCharacter))]
[RequireComponent (typeof(NavMeshAgent))]

public class ManedWolfAIBasic : MonoBehaviour {

	// cmcb  2017/05/30

	// Basic AI is mostly about whether the npc needs to rest.

	private ManedWolfCharacter ctlagent;
	...
	private class AISleepRest {
		...
		private ManedWolfCharacter agent;
		...
		public void SetAnimatorAgent(ManedWolfCharacter anim) {
	...
	private abstract class AIAction{
		...
		protected ManedWolfCharacter ctlagent;
	...
	private class AIPatrolOnce : AIPatrol {
		...
		protected override void Patrol() {
			...
			// Debug.Log("AIPatrolOnce-->Patrol()");
			// Debug.Log("Waypoint = " + currentWaypoint );
			if ( HaveArrived(waypoints[currentWaypoint].position)) {
				currentWaypoint++;
				if (currentWaypoint >= waypoints.Length){
					finished = true;
					// Debug.Log ("Patrol finished.  Resting now...");
					restagent.StartRest();
				}
				else {
					MoveTo(waypoints[currentWaypoint].position);
	...
	void Start () {
		...
		ctlagent = GetComponent<ManedWolfCharacter>();
	...
	private void StartDoing() {
		...
		// Debug.Log (this.name + " starting doing...(" + diceroll + ")");
